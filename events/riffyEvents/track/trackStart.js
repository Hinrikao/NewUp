...
client.riffy.on('trackStart', async (player, track) => {
    ...
    const trackDuration = track.info.isStream ? "AO VIVO" : formatDuration(track.info.length);
    const trackAuthor = track.info.author ? authors : "Desconhecido";
    const trackTitle = track.info.title ? titles : "Desconhecido";
    ...
    const startembed = new EmbedBuilder()
		...
		.addFields(
			{ name: "Artista", value: `${trackAuthor}`, inline: true },
			{ name: "Duração", value: `\`${trackDuration}\``, inline: true },
			{ name: "Solicitado por", value: `${track.info.requester}`, inline: true },	
		)
        .setFooter({ text: `Loop: ${capital(player.loop)} • Fila: ${player.queue.length} música(s) • Volume: ${player.volume}%` });
    ...
    const filter = (message) => {
        if (message.guild.members.me.voice.channel && message.guild.members.me.voice.channelId === message.member.voice.channelId)
            ...
            message.reply({
                content: `\`❌\` | Você precisa estar no mesmo canal de voz que eu para usar este botão.`,
                ephemeral: true,
            });
        }
    };
        ...
        if (message.customId === "loop") {
            if (!player) {
                ...
                const embed = new EmbedBuilder()
                    .setDescription(`\`✔️\` | Modo de loop definido para: \`${player.loop}\``)
                    .setColor(default_color);
                ...
                const loopEmbed = new EmbedBuilder()
                    ...
                    .setFooter({
                        text: `Loop: ${capital(player.loop)} • Fila: ${player.queue.length} música(s) • Volume: ${player.volume}%`,
                    });
                ...
                const embed = new EmbedBuilder().setDescription(`\`✔️\` | Modo de loop definido para: \`${player.loop}\``).setColor(default_color);
                message.reply({ embeds: [embed], ephemeral: true });
                const loopEmbed = new EmbedBuilder()
                .setAuthor({
                    name: `Tocando Agora`,
            ...
            if (!player) {
                message.reply({ content: `\`❌\` | O player não existe`, ephemeral: true })
                collector.stop();
            } else {
                await player.seek(0);
                const embed = new EmbedBuilder().setDescription('\`✔️\` | A música foi reiniciada').setColor(default_color);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder().setDescription(`\`❌\` | Fila está: \`Vazia\``).setColor(default_color);
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder()
                .setDescription(`\`❌\` | Volume não pode ser menor que: \`10%\``)
                .setColor(default_color);
            ...
            await message.reply({ 
                embeds: [new EmbedBuilder().setDescription(`\`✔️\` | Volume diminuído para: \`${player.volume}%\``).setColor(default_color)], 
                ephemeral: true 
            });
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder()
                .setDescription(`\`❌\` | Volume não pode ser maior que: \`150%\``)
                .setColor(default_color);
            ...
            await message.reply({ 
                embeds: [new EmbedBuilder().setDescription(`\`✔️\` | Volume aumentado para: \`${player.volume}%\``).setColor(default_color)], 
                ephemeral: true 
            });
            ...
            if (!player) {
                ...
                const embed = new EmbedBuilder()
                    .setDescription(`\`♾\` | Reprodução automática está \`ativada\``)
                    .setColor(default_color);
                ...
                const embed = new EmbedBuilder()
                    .setDescription(`\`♾\` | Reprodução automática está \`desativada\``)
                    .setColor(default_color);
